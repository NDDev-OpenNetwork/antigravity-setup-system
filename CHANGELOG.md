...
The Antigravity CLI provider retains the existing protocol and preservation contract.
...
The Antigravity CLI provider retains prior setup identity and written ownership.
...
declares.** The rule a model reads while working on Antigravity CLI said the product
...
**A walk written when a surface held one kind.** Antigravity CLI's table gained a
...
**Antigravity CLI's table gained a kind.** Where a product loads a plugin from a
...
**Antigravity CLI declares the `instruction` kind**, and the surface that routes it is
...
Antigravity CLI's provider publishes a projection profile per *scope* -- a second
...
Starts Antigravity CLI, and takes over a target the estate before this one
still claims.
...
Installs Antigravity CLI itself, not only its configuration.
...
Antigravity CLI harness configuration in a caller-named target directory.
