...
Installs, reselects, restores and removes a complete Antigravity CLI harness configuration, and owns the program lifecycle.
...
antigravity-setup-system select minimal      --target ~/.tool-config
...
antigravity-setup-system restore --backup slot-000000000002 --target ~/.tool-config
...
Point `ANTIGRAVITY_SETUP_SYSTEM_SETUP_CATALOG` at a directory to use setups of your own.
...
```bash
ai-stp provider conformance --harness antigravity \
  --executable ~/.local/bin/antigravity-setup-system \
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/antigravity-setup-system:0.0.2 \
