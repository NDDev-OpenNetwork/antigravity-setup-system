...
Corrected 2026-08-29. The previous text read *"which one the product prefers when both exist is not documented and has not been measured"* — true when written, and false against a changelog that was inside the artifact this baseline already pins. Found because a user's issue about a different hooks path sent someone reading the same bytes again. ([source](measured from the pinned 1.1.22 binary; no vendor page names it))
...
Absence of a literal is not proof a path cannot exist. This row says what was searched. ([source](measured in the pinned 1.1.22 artifact))
