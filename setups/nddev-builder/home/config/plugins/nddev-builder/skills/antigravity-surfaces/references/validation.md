...
```bash
target="$(mktemp -d)/antigravity-target"
mkdir -p "$target"
antigravity-setup-system install baseline    --target "$target"
antigravity-setup-system status              --target "$target"
antigravity-setup-system select full-auto    --target "$target"
antigravity-setup-system diff                --target "$target"
antigravity-setup-system backups             --target "$target"
antigravity-setup-system restore             --target "$target"
antigravity-setup-system remove              --target "$target"
```
...
Ask `antigravity-setup-system provider-info` for `harness_id`; that is the value
`--harness` takes, and it is not always the directory name.

```bash
ai-stp provider conformance --harness <harness_id> \
  --executable target/release/antigravity-setup-system \
